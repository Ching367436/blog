...
不過還有一個問題：存 $2^{32}$ 種可能的 middle 會耗掉太多空間然後噴錯（有人還把這個做成[梗圖](/ais3-pre-exam-2023-write-up/2des-meme.png)）。
...
這個 payload 也可以通過第 6 行的檢查，所以就能成功登進去 `admin` 的帳戶了。有人一直繞不過第 6 行的檢查還被做成[梗圖](/ais3-pre-exam-2023-write-up/login-panel-meme.png)。
...
![](/ais3-pre-exam-2023-write-up/webhook.png)
